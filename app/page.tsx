...
export default function HomePage() {
  return (
    <>
      ...
      {/* Method */}
      <Section id="method" className="surface-white">
        ...
        <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            },
            {
              ...
              cta: "Tell us your goals → get a proposal",
            },
          ].map((card) => (
            <article
              key={card.title}
              className={`card p-6 ${card.featured ? "border-2 border-black shadow-sm" : ""}`}
            >
              <h3 className="text-lg font-semibold">{card.title}</h3>
              <ul className="mt-3 space-y-2 text-sm text-subtle list-disc pl-4">
                {card.bullets.map((b) => <li key={b}>{b}</li>)}
              </ul>
              {"price" in card ? (
                <p className="mt-4 font-semibold">{card.price}</p>
              ) : (
                <p className="mt-4">
                  <a href="#contact" className="underline hover:no-underline">{card.cta}</a>
                </p>
              )}
            </article>
          ))}
      ...
      <Section id="about" className="surface-white">
        ...
        <p className="mt-4 text-subtle leading-relaxed max-w-3xl">
          With over two decades of practice, Dejan Radinović has advised the Government of Montenegro and
          leading international corporations on some of the country’s most significant privatizations and
          investment projects. His track record includes high-stakes due diligence exercises, complex M&A
          transactions, and representation in international arbitration disputes exceeding €800 million.
        </p>
        <p className="mt-4 text-subtle leading-relaxed max-w-3xl">
          Dejan combines deep knowledge of Montenegrin law with an understanding of EU regulatory standards,
          offering investors rare clarity at the intersection of local rules and cross-border business. This
          background ensures that each report delivered by MNE Consulting is both source-anchored and board-ready —
          a decisive advantage for EU clients evaluating opportunities in Montenegro.
        </p>
